...
class SettingsTab:
    def __init__(self, root, frame):
        ...
        # API Type
        ttk.Label(frame, text="API Type:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.api_type = ttk.Combobox(frame, state="readonly", values=["azure", "openai", "gemini"])
        self.api_type.pack(fill=tk.X, padx=10, pady=10)
        self.api_type.bind('<<ComboboxSelected>>', self.api_type_changed)

        # API Key
        ttk.Label(frame, text="API Key:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.api_key_entry = ttk.Entry(frame)
        self.api_key_entry.pack(fill=tk.X, padx=10, pady=10)

        # API Host URL
        self.api_host_label = ttk.Label(frame, text="API Host URL:", style='W.Label')
        self.api_host_label.pack(fill=tk.X, padx=10, pady=2)
        self.api_host_entry = ttk.Entry(frame)
        self.api_host_entry.pack(fill=tk.X, padx=10, pady=10)

        # API Version
        self.api_version_label = ttk.Label(frame, text="API Version:", style='W.Label')
        self.api_version_label.pack(fill=tk.X, padx=10, pady=2)
        self.api_version_entry = ttk.Entry(frame)
        self.api_version_entry.pack(fill=tk.X, padx=10, pady=10)

        # GPT Model Name
        ttk.Label(frame, text="GPT Model Name:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.gpt_model = ttk.Entry(frame)
        self.gpt_model.pack(fill=tk.X, padx=10, pady=10)

        # GPT Deployment Name
        self.gpt_deployment_label = ttk.Label(frame, text="GPT Deployment Name:", style='W.Label')
        self.gpt_deployment_label.pack(fill=tk.X, padx=10, pady=2)
        self.gpt_deployment = ttk.Entry(frame)
        self.gpt_deployment.pack(fill=tk.X, padx=10, pady=10)

        # Embeddings Model Name
        ttk.Label(frame, text="Embed Model Name:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.embeddings_model_entry = ttk.Entry(frame)
        self.embeddings_model_entry.pack(fill=tk.X, padx=10, pady=10)

        # Embeddings Deployment Name
        self.embeddings_deployment_label = ttk.Label(frame, text="Embed Deployment Name:", style='W.Label')
        self.embeddings_deployment_label.pack(fill=tk.X, padx=10, pady=2)
        self.embeddings_deployment_entry = ttk.Entry(frame, width=90)
        self.embeddings_deployment_entry.pack(fill=tk.X, padx=10, pady=10)

        # Prompt
        ttk.Label(frame, text="Prompt:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        ...
        self.prompt_entry.pack(fill=tk.X, padx=10, pady=10)

        # Optional Settings
        ttk.Label(frame, text="Max input token length:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.max_tokens = ttk.Entry(frame)
        self.max_tokens.pack(fill=tk.X, padx=10, pady=10)
        self.max_tokens.insert(INSERT, "64000")

        ttk.Label(frame, text="Number of threads:", style='W.Label').pack(fill=tk.X, padx=10, pady=2)
        self.threads = ttk.Entry(frame)
        self.threads.insert(INSERT, "1")
        self.threads.pack(fill=tk.X, padx=10, pady=10)
    ...
    def show_only_relevant_settings(self):
        if self.api_type.get() == "openai"\
           or self.api_type.get() == "gemini":
            self.api_host_label.pack_forget()
            self.api_host_entry.pack_forget()
            self.api_version_label.pack_forget()
            self.api_version_entry.pack_forget()
            self.gpt_deployment_label.pack_forget()
            self.gpt_deployment.pack_forget()
            self.embeddings_deployment_label.pack_forget()
            self.embeddings_deployment_entry.pack_forget()
